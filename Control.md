...
The current reference implementation is in `nero_lab.py`, build `2026-09-06-powered-release-59`.
...
- The GUI enables `Amplify gripper during replay` by default for both replay flows. Amplification preserves every taught width instead of replacing the gripper path with binary 0.0/0.1 m positions. A taught width at or below 0.085 m starts a grasp interval. Reopening requires the taught width to remain near fully open for 0.45 seconds. The threshold is the recording's calibrated 99th-percentile open reference minus 1 mm, capped at the nominal 0.0994 m limit, so a legitimate small-object release plateau is not erased by slightly higher later openings. Because preprocessing has the complete recording, it uses the 0.45-second plateau as lookahead confirmation and schedules the powered release at the plateau's beginning instead of adding 0.45 seconds to the taught release. Angle-mode recordings retain their taught values.
...
- Amplified replay follows the taught gripper positions and applies 3.0 N throughout each detected grasp interval. Stable duplicate positions are suppressed, so it does not repeatedly command an unchanged gripper. Release-pose feedback is compared with the nearest reachable command-limit pose, preventing an out-of-envelope taught joint such as joint 2 at -1.759 rad from making the release gate impossible, while the original taught arm targets remain unchanged.
