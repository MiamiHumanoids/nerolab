...
The current reference implementation is in `nero_lab.py`, build `2026-09-06-tight-grasp-release-61`.
...
- Taught-task replay uses the AGX API's maximum documented actuator force of 3.0 N for both grasping and opening in normal Replay and Replay-and-Record flows. Opening also needs sufficient actuator effort to release a loaded gripper reliably. The previous 30.0 N value exceeded the documented 0.0-3.0 N range and could make the driver relax after its initial close attempt.
- The GUI enables `Amplify gripper during replay` by default for both replay flows. Amplification preserves taught widths during approach. A taught width at or below 0.085 m starts a grasp interval and issues one tight 0.0 m command at the maximum valid 3.0 N; unchanged grasp commands are then suppressed. Reopening requires the taught width to remain near fully open for 0.45 seconds. The threshold is the recording's calibrated 99th-percentile open reference minus 1 mm, capped at the nominal 0.0994 m limit, so a legitimate small-object release plateau is not erased by slightly higher later openings. Because preprocessing has the complete recording, it uses the 0.45-second plateau as lookahead confirmation and schedules a powered 0.1 m full-clearance release at the plateau's beginning. It holds that open target until the next taught grasp so near-open sensor values cannot leave a small object pinched. Angle-mode recordings retain their taught values.
...
- Amplified replay uses one 0.0 m, 3.0 N command throughout each detected grasp interval. Stable duplicate positions are suppressed, so it does not repeatedly command an unchanged gripper. Release-pose feedback is compared with the nearest reachable command-limit pose, preventing an out-of-envelope taught joint such as joint 2 at -1.759 rad from making the release gate impossible, while the original taught arm targets remain unchanged.
